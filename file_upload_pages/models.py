...
class newFile():
   def __init__(self, tempFile, tempFileName):
      ...
      self.imagesFolder = settings.MEDIA_ROOT + "/images/"
      self.thumbnailsFolder =  settings.MEDIA_ROOT + "/thumbnails/"
   ...
   def storeThumbnail(self):
      self.fileToUpload.seek(0)
      uploadingImage = Image.open(io.BytesIO(self.fileToUpload.read()))
   ...
   def uploadImage(self):
      ...
      save_path = join(self.imagesFolder, self.fileToUploadName)
      print(settings.MEDIA_ROOT)
      print(save_path)
      path = default_storage.save(save_path, self.fileToUpload)
   ...
   def alreadyExists(self):
      ...
      imageExists = False
      self.fileToUpload.seek(0)
      checkingImage = Image.open(io.BytesIO(self.fileToUpload.read()))
...
class imageList():
   def __init__(self):
      self.imagesFolder = settings.MEDIA_ROOT + "/images/"
      self.thumbnailsFolder = settings.MEDIA_ROOT + "/thumbnails/"
      self.imagesURL = "/data/images/"
      self.thumbnailsURL = "/data/thumbnails/"
   def __del__(self):
      self.imagesFolder = None
      self.thumbnailsFolder = None
   ...
   def getList(self):
      ...
      for existingImage in existingImages:
         imagesData = {}
         imagesData["thumbnail"] = self.thumbnailsURL + basename(existingThumbnails[imageCount]) if existingThumbnails[imageCount] else ""
         imagesData["normal"] = self.imagesURL + basename(existingImage) if existingImage else ""
